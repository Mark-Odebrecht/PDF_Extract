...
IS_EXIST = os.path.exists(STR_TIME)
...
for ticker in tickers:
  ...
  # df = get_symbols(tickers, start, end)
    ...
    # Creating column 'Positions' to add -1 where the ratio is greater 
    # the 85th percentile (sell) and NaN otherwise
    df['Positions'] = np.where(df['Ratios'] > sell, -1, np.nan)

    # Adding 1 where the ratio is less than the 15th percentile (buy) 
    # and put the current value otherwise
    ...
    label = 'Bollinger sell signal', marker = '1', alpha = 1, s=200)
    # ax1.legend()
    ...
    plt.savefig(os.path.join(PATH, ticker+' - '+STR_TIME+'.png'))
